...
export class ProgramCinema implements OnInit {
  ...
  genereazaZileSaptamana() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      zile.push({
        eticheta: esteAstazi
          ? 'Astăzi'
          : zileShort[d.getDay() === 0 ? 6 : d.getDay() - 1],
        date: d,
      });
    }
  ...
  getFilmeFiltrate() {
    return this.filmeAfisate.filter((film) => {
      ...
      const potrivesteTitlu = this.filmSelectat
        ? film.name === this.filmSelectat
        : true;
  ...
  moveRight() {
    this.currentIndex = (this.currentIndex + 1) % this.filmeCarousel.length;
  }
